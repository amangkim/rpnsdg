function NVSDG = nvsdgbestshooting(varargin)
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here

%-----------------------------------------------
inputs={'SucPro', 'Time', 'Shooter_idx', 'Bullet', 'DispOtp', 'Ply_nm'};
Time = [];
Shooter_idx = 1;
Ply_nm = [];
DispOtp = [];
...
if DispOtp >= 1 %-----------------------------
    ...
    for i4 =1:nd
        eply = Ply1(i4);
        t0 = t1(i4);
        pstar = p0(t_opt_idx(i4));
        %eachply = ['with Player-' num2str(eply) ' @ ' num2str(t0, '%4.2e')];
        eachply = ['with Player-' num2str(eply) ', [',num2str(pstar,'%1.2f'),' @' num2str(t0, '%2.2f') ']'];
        player_leg = [player_leg; eachply];
    end
    %-------------------------------------------    
    
    %player_leg
    eachply0 = ['Base: Player-' num2str(Ply0)];
